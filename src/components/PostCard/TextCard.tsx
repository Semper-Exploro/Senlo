...
export function TextCard({ post }: CardProps) {
  ...
  return (
    <article className="bg-white rounded-xl overflow-hidden border border-gray-100 hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5 group p-4">
      {/* 标题（如果有） */}
      {post.title && (
        <h3 className="text-[15px] font-semibold text-gray-800 leading-snug mb-2">
          {post.title}
        </h3>
      )}

      {/* 内容 */}
      <p className="text-[14px] text-gray-700 leading-relaxed line-clamp-4 mb-3">
        {post.content}
      </p>
